...
class AgentBridgeProcess {
  ...
  private pending: Array<{ resolve: (value: any) => void; timer: NodeJS.Timeout }> = [];
  private stderrBuffer = "";
  private stdoutBuffer = "";
  private lastError = "";

  public start(workspaceRoot: string, pythonPath: string, bridgeScript: string): void {
    if (this.proc) return;
  ...
  public async request(payload: Record<string, unknown>, timeoutMs = 60000): Promise<any> {
    if (this.lastError) {
...
class VVSAgantViewProvider implements vscode.WebviewViewProvider {
  public static readonly viewType = "vvsagant.chatView";
  private _view?: vscode.WebviewView;
  private bridge = new AgentBridgeProcess();
  ...
  public resolveWebviewView(
    ...
    webviewView.webview.onDidReceiveMessage(async (msg) => {
      ...
      if (type === "ask") {
        ...
        try {
          if (!this.bridge.isRunning) throw new Error("桥接未连接");
      ...
      if (type === "tool_approval") {
        ...
        if (!["allow", "skip"].includes(decision)) return;
  ...
  private sendConfigToView(data: any): void {
  ...
  private isConfigReady(entry: any): boolean {
  ...
  private async handleSaveConfig(msg: any): Promise<void> {
    try {
...
export function activate(context: vscode.ExtensionContext): void {
  ...
  context.subscriptions.push(
    vscode.window.registerWebviewViewProvider(VVSAgantViewProvider.viewType, provider, {
      webviewOptions: { retainContextWhenHidden: true },
    }),
  );
  context.subscriptions.push(
    vscode.commands.registerCommand("vvsagant.open", () => {
      vscode.commands.executeCommand("workbench.view.extension.vvsagant-sidebar");
    }),
  );
  context.subscriptions.push(
    vscode.commands.registerCommand("vvsagant.reset", () => {
      vscode.commands.executeCommand("workbench.view.extension.vvsagant-sidebar");
    }),
  );
}
